...
static struct argp_option portfolio_options[] = {
        { "portfolio", 'p', "file", 0, "use given portfolio file", 0},
        { 0 }
};
...
static struct argp list_argp = { list_options, list_parse_opt, 0, list_doc, 0, 0, 0 };
static struct argp stats_argp = { stats_options, stats_parse_opt, 0, stats_doc, 0, 0, 0 };
static struct argp portfolio_argp = { portfolio_options, portfolio_parse_opt, "[PORTFOLIO]", portfolio_doc, 0, 0, 0 };
...
void parse_subcommand(struct argp_state* state, const char* name, struct argp* argp) {
    ...
    switch (key) {
        ...
        case ARGP_KEY_ARG:
            for (size_t i = 0; i < SUBCOMMANDS_SIZE; i++) {
                if (strncmp(arg, subcommands[i].name, strlen(subcommands[i].name)) == 0) {
